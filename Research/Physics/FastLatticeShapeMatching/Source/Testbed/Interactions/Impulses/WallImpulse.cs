...
namespace PhysicsTestbed
{
    public class WallImpulse : EnvironmentImpulse
	{
        ...
        public override void ApplyImpulse(LsmBody applyBody, Particle applyParticle, Vector2 pos, Vector2 posNext, Vector2 velocity, ref List<CollisionSubframeBuffer> collisionBuffer, double accumulatedSubframeTime)
		{
            ...
            if (posNext.X < left)
            {
                collisionBuffer.Add(new CollisionSubframeBuffer(new Vector2(-velocity.X, velocity.Y), (left - pos.X) / velocity.X));
            }
            if (posNext.X > right)
            {
                collisionBuffer.Add(new CollisionSubframeBuffer(new Vector2(-velocity.X, velocity.Y), (right - pos.X) / velocity.X));
            }
            if (posNext.Y < bottom)
            {
                collisionBuffer.Add(new CollisionSubframeBuffer(new Vector2(velocity.X, -velocity.Y), (bottom - pos.Y) / velocity.Y));
            }
            if (posNext.Y > top)
            {
                collisionBuffer.Add(new CollisionSubframeBuffer(new Vector2(velocity.X, -velocity.Y), (top - pos.Y) / velocity.Y));
            }
        }
	}
}
